...
The framework provides features to allow your application to be targeted towards different resolutions. You will find some devices (e.g. TVs) run in a fixed resolution, whilst others, (e.g. Game Consoles) can run in different resolutions depending on user settings.
...
| `.animating`              | Applied to the TLE during an animation caused by a Device.\* call.  |
| `.notanimating`           | Applied to the TLE when an animation has finished.  |
...
*   The framework adds an `.animating` and `.notanimating` class name to the TLE when animations are occurring or are idle.
*   If targeting devices that play video behind the browser, you may want to add a `.playing` or `.backgroundPlaying` class name to the TLE whilst video is playing, allowing you to remove any body/application background images/colours that may obscure the video.
...
The default build scripts make use of [Smart Sprites|http://csssprites.org/] to allow optimised sprite images to be created at build time. Making use of Smart Sprites is easy.
